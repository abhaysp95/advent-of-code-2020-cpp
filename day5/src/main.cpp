...
void part2() {
	...
	std::cout << "My seat is: " << my_seat_number << std::endl;
	input_file.close();
}
...
void more() {
	...
	std::map<std::string, size_t> seats_info;
	std::string pos_str;
	while (input_file >> pos_str) {
		std::string pos_row(pos_str.begin(), (pos_str.begin() + 7));
		std::string pos_col(pos_str.begin() + 7, pos_str.end());
		...
		size_t get_col_pos = get_place(pos_col_begin, pos_col_end, 0, 7, 'L', 'R');
		size_t seat_id = get_row_pos * 8 + get_col_pos;
		seats_info.insert(std::pair<std::string, size_t>(pos_str, seat_id));
	}
	std::string my_seat_number = get_my_seat_path(seats_info);
	if (!my_seat_number.empty()) {
		std::cout << "My seat is: " << my_seat_number << std::endl;
	}
	input_file.close();
}
